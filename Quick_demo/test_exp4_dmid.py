...
def main():
    ...
    birad_csv = "dmid_gt.csv"

    with open(image_csv, "r") as f_img, open(birad_csv, "r") as f_gt, open("output_exp4_dmid.csv", "w", newline="") as outf:
        img_reader = csv.DictReader(f_img)
        gt_reader = csv.reader(f_gt)
        writer = csv.writer(outf)

        # Write header
        writer.writerow(["image_path", "birad", "acr", "prompt", "prediction"])

        # Skip header of GT CSV
        next(gt_reader)

        # Iterate sequentially over both files
        for row_img, row_gt in zip(img_reader, gt_reader):
            img_path = row_img["image_path"]
            birad_val = row_gt[1]
            acr_val = row_gt[2]
            ...
            try:
                question = (
                    ...
                    f"The BI-RADS score is {birad_val} and the ACR density is {acr_val}. "
                    f"Generate a radiology report for this mammogram."
                )
                ...
                writer.writerow([img_path, birad_val, acr_val, question, pred])
            except Exception as e:
                writer.writerow([img_path, birad_val, acr_val, question, f"[ERROR: {e}]"])

            print("Testing completed for image:", img_path)

    print("Saved predictions to output_exp4_dmid.csv")
